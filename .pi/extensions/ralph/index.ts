...
/**
 * Ticket IDs assigned to `@human`, across every status.
 *
 * This project targets physical hardware, so some tickets can only be finished by a
 * person: flashing a board, listening for audio, recording instruments, or making an
 * owner's decision like creating a repo or posting upstream. Their acceptance criteria
 * are prefixed "HUMAN:" precisely because no amount of agent work satisfies them — an
 * agent that picks one up cannot succeed, it can only report a false Done off the back
 * of a clean compile. See the assignment convention in CLAUDE.md.
 *
 * Tickets with NO assignee stay eligible; only an explicit `@human` is excluded.
 *
 * Deliberately re-read per call rather than cached on the loop state: this is a fast
 * local command, and a stale set would silently hand a human ticket to the executor —
 * the exact failure this exists to prevent. Correctness over three saved execs.
 */
async function listHumanAssignedIds(
  pi: ExtensionAPI,
  cwd: string,
): Promise<Set<string>> {
...
async function findFirstByStatus(
  ...
  // Filtered here rather than at the call sites so every consumer — execute, plan,
  // and anything added later — inherits the guarantee without having to remember it.
  const humanOnly = await listHumanAssignedIds(pi, cwd);
  return parsePlainTaskList(stdout).find((t) => !humanOnly.has(t.id));
}
...
/**
 * Candidates for `choose`. Excludes `@human` tickets — unlike
 * `promoteUnblockedBlockedTickets`, which deliberately does NOT filter: promoting a
 * human ticket out of Blocked once its dependencies land is useful to the person, it
 * just must not then be selected for agent work.
 */
async function listUnblocked(pi: ExtensionAPI, cwd: string): Promise<Ticket[]> {
  const [candidates, humanOnly] = await Promise.all([
    listUnblockedByStatus(pi, cwd, "To Do"),
    listHumanAssignedIds(pi, cwd),
  ]);
  return candidates.filter((t) => !humanOnly.has(t.id));
}
